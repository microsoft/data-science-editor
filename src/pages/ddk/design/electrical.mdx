...
Current limiters specify a tolerance on their output current - please ensure that the **maximum** output current does not exceed 150mA or 1A (for low current and high current providers respectively). This often means setting the typical current to a value like 100mA or 900mA, which might result in as little as 75mA or 800mA in some cases. But it is important that the **maximum** current specification is not exceeded.

It is possible for a high current provider to have a maximum current of any value between 150mA and 1A. For example, a battery pack that is only able to supply 500mA could be used with a 500mA maximum Jacdac power provider.
...
Jacdac requires cabling of at least 26AWG, resulting in a resistance of 150mΩ/m. In addition, the contact resistance in a Jacdac connector may be up to 30mΩ. Therefore cables have a total end-to-end resistance of (150L+60) mΩ where L is the cable length in m. For example, a 350mm single-hop cable will have up to 113mΩ resistance including contact resistance at both ends.

### Bus voltage range

The nominal 5V bus voltage and maximum 1A bus current limit allow commonly available USB power delivery parts to be used for Jacdac. In particular, 5V power adapters, 5V power packs, 5V current limiting ICs and 5V DC-DC converters are all commodity items. Note that high current providers do not have to provide 1A, it may for example be cheaper to implement a 500mA power provider.

Note that any current flowing through the Jacdac bus will result in a voltage drop between the points of power provision and consumption, and any ground return current will cause a similar voltage drop. At 1A and with a 350mm,113mΩ cable, this results in a 113mV drop on JD_PWR between power provider and power consumer, and another 113mV 'rise' on the GND return path; the power consuming device sees 226mV less than is being provided at the source. Additional hubs and cables will further reduce the margin.

### Power consumers

A power consumer may draw an absolute maximum of 1A from the Jacdac bus, however we recommend a maximum consumption of 900mA given that 1A may not be available due to component tolerances.

To accommodate voltage drops, Jacdac devices must be capable of operating when the potential between JD_PWR and GND falls to 3.6V. Since we expect the logic of a Jacdac device to operate at 3.3V (or lower), a low dropout linear regulator is the cheapest and simplest way to regulate JD_PWR to power a device from the bus. Alternatively a DC-DC converter can be used for improved efficiency.
