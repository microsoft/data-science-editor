...
You can load CSV file hosted on internet using `load dataset from url`. The data must be CSV formated with header names.

It is possible to import CSV files using `load dataset from file` (Chrome/Edge only).
