...
For module with a fixed, small number of LEDs, see the [LED display](/clients/makecode/extensions/leddisplay/) service.
...
```blocks
input.onButtonPressed(Button.A, () => {
    modules.ledStrip1.setAll(0)
    let n = modules.ledStrip1.numPixels()
    for(let i = 0; i <= n - 1; i++) {
        modules.ledStrip1.setPixel(i, 0x00ff00)
        basic.pause(200)
    }
})
```
