...
https://www.youtube.com/watch?v=rlK_8oqMAmo
...
-   if you cannot find this block, start a **button simulator** in Jacdac and click **ADD BLOCKS** button
...
To end the game, let's add a `if then else` block that only allows incrementing
the counter for the first 10 seconds of the micro:bit running.

-   drag a `if then else` block in the down event and move the current blocks under the `then` section
-   add code that checks that `running time (ms)` is less (<) than `10000` (ms).
...
<MakeCodeProjectFooter serviceNames="button" />
