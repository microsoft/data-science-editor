...
Before starting these steps, please make sure your **micro:bit V2** is connected to your computer using a USB cable. Please also ensure your micro:bit is running [the out of box firmware](/hardware/kit/oob/) it came with!
...
Grab the `on slider position changed by 5` from the modules block category. Then, take a `plot bar graph` block from the "Led" category and place it inside the `on slider position changed by 5` block.
...
From the modules drawer, grab the "potentiometer1 position" and place it on the first "0" of the "plot bar graph" block:
...
That's it! Now, when you move the slider position, you will see a poportional change to the number of Leds displayed on the micro:bit display.

![a screenshot of a user changing the slider and the simulator updating](./makecode-slider-proportional-change.gif)

### (optional) Step 7

Just like any MakeCode program, you can click the download button to see the same program running on your physical micro:bit!
...
```blocks
modules.ledPixel1.setAll(input.acceleration(Dimension.X))
```
