...
The Jacdac Device Development Kit (DDK) is for hardware designers, firmware developers and manufacturers who wish to create their own Jacdac [devices](/devices). A Jacdac device should:
...
There are four main types of Jacdac device: brains, modules, power supplies and adaptors (or Jacdaptors). More information about these is desscribed along with Jacdac's [client/server architecture](/reference/clientserver).
...
Hardware reference implementations of Jacdac devices created with popular electronics CAD tools including Altium Designer, Eagle and KiCad are available at https://github.com/microsoft/jacdac-ddk.
